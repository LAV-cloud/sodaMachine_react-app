...
const App = () => {
  ...
  async function getSodas() {
    try {
      await fetch(`http://localhost:4200/sodas`)
        .then((res) => res.json())
  ...
  const addSoda = (icon, name, price, count) => {
    let newSoda = {
    ...
    setSodas((sodas = [...sodas, newSoda]));
    sodas.map((soda) => {
      if (soda.id === newSoda.id) {
        success = true;
      }
      return soda;
    });
    ...
    if (success) {
      addNot(0, 'Successful create new soda');
    } else {
      addNot(1, 'Unsuccessful create new soda');
    }
  };
